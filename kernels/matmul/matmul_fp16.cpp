...
typedef half     data_t;   // fp16 storage/compute
typedef half     acc_t;    // fp16 accumulation
typedef uint32_t pack_t;   // 2x half packed = native 32-bit AXI word

struct row_chunk_t {
    data_t v[TILE];
};

void load_rows(const pack_t* __restrict__ A, hls::stream<row_chunk_t> &s_row) {
...
#pragma HLS PIPELINE II=1
                // one 32-bit AXI beat = 2 packed halfs, native bus width
                pack_t packed = A[i * (DIM/2) + k * (TILE/2) + t];
                data_t lo, hi;
                uint16_t lo16 = packed & 0xFFFF;
                uint16_t hi16 = (packed >> 16) & 0xFFFF;
                lo = *reinterpret_cast<data_t*>(&lo16);
                hi = *reinterpret_cast<data_t*>(&hi16);
                chunk.v[2*t]     = lo;
                chunk.v[2*t + 1] = hi;
            }
            s_row.write(chunk);
...
extern "C" void matmul_fp16(
    const pack_t* __restrict__ A,   // packed 2xhalf per 32-bit word
    const data_t* __restrict__ x,
    acc_t*        __restrict__ y
) {
#pragma HLS INTERFACE m_axi port=A offset=slave bundle=gmem0 depth=(DIM*DIM/2) max_read_burst_length=16
#pragma HLS INTERFACE m_axi port=x offset=slave bundle=gmem1 depth=DIM max_read_burst_length=16
#pragma HLS INTERFACE m_axi port=y offset=slave bundle=gmem2 depth=DIM max_write_burst_length=16
