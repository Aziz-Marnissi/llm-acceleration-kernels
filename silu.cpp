...
#define LUT_MAX   8.0f
#define TILE     1024
#define NTILES_MAX 64   // borne haute pour TOTAL_LEN/TILE (ajuste si besoin)
...
#pragma HLS INLINE
    int idx = (int)((x - LUT_MIN) * (data_t)(LUT_SIZE / (LUT_MAX - LUT_MIN)));
    if (idx < 0)
        idx = 0;
...
void silu(const data_t *gate,
          ...
          int n)
{
#pragma HLS INTERFACE m_axi port=gate offset=slave bundle=gmem0 depth=38912
#pragma HLS INTERFACE m_axi port=up   offset=slave bundle=gmem1 depth=38912
#pragma HLS INTERFACE m_axi port=out  offset=slave bundle=gmem2 depth=38912
